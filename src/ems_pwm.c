...
#define PWM_MODES_TIMER_DEF                     NRFX_TIMER_INSTANCE(1) // Timer1
...
#else 
#define CHAN1_PWM_GPIO_PIN                      15
#endif

#define CHAN1_PWM_INST                          NRFX_PWM_INSTANCE(0)
...
#else
#define CHAN2_PWM_GPIO_PIN                      19
#endif

#if (CONFIG_BOARD_EMS_NRF52840)
#define CHAN3_PWM_GPIO_PIN                      26
#else
#define CHAN3_PWM_GPIO_PIN                      22
#endif

#define CHAN23_PWM_INST                         NRFX_PWM_INSTANCE(1)

#define CHAN23_DEFAULT_DURATION_NS              PWM_MSEC(10)

#define NRFX_PWM_PIN_NOT_USED                 25 
...
// Timer for channel 0
const nrfx_timer_t pwm_modes_timer = PWM_MODES_TIMER_DEF;
        ...
        if (chan0_spec.status == 1) { // Inactive period
            ...
            if (chan1_spec.status == 1) {
                ...
                if (((PWM_MSEC(ems_intensity_data.PWM_DURATION) / NSEC_PER_MSEC) - chan1_counter_on) == ems_intensity_data.CHAN0_INDUCTOR) {
                    ...
                    if (!nrfx_pwm_is_stopped(&chan23_pwm_inst)) {
...
// Function to trigger single playback on CHAN23 without activating CHAN0 and CHAN1
int ems_pwm_single_playback_chan23(uint8_t active_channel) {
    // Configure the active channel (either channel 2 or 3)
    if (active_channel == 2) {
        chan23_pwm_seq_ind_values.channel_0 = chan2_spec.pwm_on_time_ns / NSEC_PER_USEC;
        chan23_pwm_seq_ind_values.channel_1 = PWM_USEC(ems_intensity_data.PWM23_DURATION) / NSEC_PER_USEC;
    } else if (active_channel == 3) {
        chan23_pwm_seq_ind_values.channel_0 = PWM_USEC(ems_intensity_data.PWM23_DURATION) / NSEC_PER_USEC;
        chan23_pwm_seq_ind_values.channel_1 = chan3_spec.pwm_on_time_ns / NSEC_PER_USEC;
    } else {
        LOG_ERR("Invalid channel specified for single playback");
        return -EINVAL;
    }

    // Set up sequence values for playback
    chan23_values.p_individual = &chan23_pwm_seq_ind_values;
    chan23_sequence.values = chan23_values;
    chan23_sequence.length = 4; // Using length specific to individual channels (4)

    // Set playback count to 1 for single playback
    uint16_t playback_count = 1;

    // Trigger single playback on CHAN23
    if (nrfx_pwm_simple_playback(&chan23_pwm_inst, &chan23_sequence, playback_count, NRFX_PWM_FLAG_STOP) == 0) {
        ...
        LOG_INF("Single playback started on CHAN23");
        return 0;
    }
    LOG_ERR("Single playback on CHAN23 failed");
    return -EIO;
}
...
// Initialise timer
// Timer controls channel 0 gpio pin, which in turn determines when to switch channels 1, 2, 3
static int pwm_modes_timer_init() {
    LOG_INF("Init PWM modes timer");

    int ret = 0;
    nrfx_timer_config_t pwm_modes_timer_conf = {
        .bit_width = TIMER_BITMODE_BITMODE_16Bit,
        .frequency = NRF_TIMER_FREQ_500kHz,
        .mode = TIMER_MODE_MODE_Timer,
        .interrupt_priority = NRFX_TIMER_DEFAULT_CONFIG_IRQ_PRIORITY,
    };
    ret = nrfx_timer_init(&pwm_modes_timer, &pwm_modes_timer_conf, pwm_modes_timer_event_handler);
    
    if (ret != NRFX_SUCCESS) {
        LOG_ERR("PWM modes timer init fail (err %d)", ret);
        return ret;
    }
       ...
       int ret = ems_pwm_generate_simultaneous_spikes_chan23();
        ...
        if (!nrfx_timer_is_enabled(&pwm_modes_timer)) {
           nrfx_timer_enable(&pwm_modes_timer);
            nrfx_timer_extended_compare(&pwm_modes_timer, NRF_TIMER_CC_CHANNEL0, 500, NRF_TIMER_SHORT_COMPARE0_CLEAR_MASK, true); // Every 1ms
...
// Update the duty cycle of the power signal (CHAN2)
void ems_pwm_update_power_signal_intensity(uint8_t duty_cycle) {
    if (duty_cycle > 100) {
        LOG_WRN("PS intensity out of range");
        return;
    }

    uint64_t ps_on_time_ns = 0;
    memcpy(&update_buf[1], &chan1_spec, sizeof(chan1_spec));

    if (duty_cycle > 0) {
        ps_on_time_ns = (duty_cycle * PWM_USEC(ems_intensity_data.PWM_PERIOD)) / 100;
        update_buf[1].pwm_on_time_ns = ps_on_time_ns;
    } else {
        update_buf[1].pwm_on_time_ns = 0;
    }
    chan_param_update_available |= (1 << 1);
}